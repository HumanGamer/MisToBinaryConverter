...
{
    std::vector<Token> Tokenizer::parse(const std::string &input)
    {
        ...
        for (char currCh : input)
        {
            if (currentToken.mType == STRING_ESCAPE_SEQUENCE)
            {
                switch (currCh)
                {
                    ...
                    default:
                    {
                        std::string s = std::string("unknown escape sequence: \\") +
                                        std::string(1, currCh) +
                                        " in string on line " +
    ...
    {
        std::cout << "Token(" << sTokenTypeNames[mType] << ", \"" << mText << "\", " << mLineNumber << ")" << std::endl;
    }
}
